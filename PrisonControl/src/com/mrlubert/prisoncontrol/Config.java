    ...
    private static YamlConfiguration load(String FileLocation)
    {
        ...
        if ( ! f.exists())
        {
            try 
            {
                ...
                {
                    Bukkit.getServer().getLogger().log(Level.INFO, "[PrisonControl] New Config Created at: " + FileLocation);
                    cfg.save(new File(FileLocation));
                } else
                {
                    Bukkit.getServer().getLogger().log(Level.SEVERE, "[PrisonControl] Failed to create Config file");
                }
