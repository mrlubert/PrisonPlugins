...
abstract class Pick {
	...
	static boolean isPick(ItemStack item) {
		return item != null && item.getType() == Material.DIAMOND_PICKAXE
				|| item.getType() == Material.NETHERITE_PICKAXE && item.hasItemMeta() && item.getItemMeta().hasLore();
	}
	...
	static ItemStack getDrop(int enchantmentLevel, Block block, ItemStack tool) {
		...
		if (block.getType() == Material.REDSTONE_ORE || block.getType() == Material.DEEPSLATE_REDSTONE_ORE
				|| block.getType() == Material.GLOWSTONE || block.getType() == Material.COAL_ORE
	...
	void doDamage(boolean unbreaking, Player player) {
		...
		if (((Damageable) tool.getItemMeta()) == null) {
			return;
		} else if (((Damageable) tool.getItemMeta()).getDamage() > tool.getType().getMaxDurability()) {
			...
			if (chance == 1 && player.getName() != null) {
				Bukkit.broadcastMessage(Config.CHAT_PICK_BREAK + "Uh oh, " + player.getName()
						+ " wasn't paying attention. Don't forget to right click because they broke a " + type + " "
						+ pick + Config.CHAT_PICK_BREAK + " while mining... Press 'F' to pay respects. ");
				String command = "sudo " + player.getName() + " c: [i]";
				Bukkit.dispatchCommand((CommandSender) console, command);
			} else if (chance != 1 && player.getName() != null) {
				Bukkit.broadcastMessage(Config.CHAT_PICK_BREAK + player.getName() + " just broke their " + type + " "
						+ pick + Config.CHAT_PICK_BREAK + " while mining... Press 'F' to pay respects. ");
				String command = "sudo " + player.getName() + " c: [i]";
				Bukkit.dispatchCommand((CommandSender) console, command);
			}
